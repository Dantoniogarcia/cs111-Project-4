...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	{
		error("The Filename attempting to upload is larger than the max Filename length allowed\n");
                goto exit; // i think this is right the professor does in other bad case below
...
{
	//Task 2
	//check if filename is longer than the max file length provided by the professor
	if (FILENAMESIZ < strlen(t->filename))
	{
		error("The Filename attempting to download is larger than the max Filename length allowed\n");
		goto try_again; // i think this is right the professor does in other bad case below
	}
	//Task 2 end
	int i, ret = -1;
	...
	if(evil_mode != 0)
	{
		//overload file name
		int i;
		char overloadSucker[FILENAMESIZ*5]; // making sure it is larger than the filename
		for(i = 0; i < FILENAMESIZ*5; i++)
			overloadSucker[i] = 'E'; // filll up the file name
		//keep writing to them and overloading there buffer infinitely
	//	while(1)
	//	{
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	while (1) {
		if((t->head- t->tail) > TASKBUFSIZ-1)
		{
                        error("Over the max upload size. Will try again\n");
                        goto try_again;
		}
		int ret = read_to_taskbuf(t->peer_fd, t);
		if (ret == TBUF_ERROR) {
...
{
	/*Task 2B*/char *directoryPath1; // allocating data. +1 is fo null byte
	 /*Task 2B*/ char *wantedFilePath1; // variable for path of wanted file
	char directoryPath2[PATH_MAX +1];
	char wantedFilePath2[PATH_MAX+1];
	assert(t->type == TASK_UPLOAD);
	if(strlen(t->filename) > FILENAMESIZ)
		error("File name requested for upload is too long\n");
	...
	while (1) {
		//unsigned headpos = (t->head % TASKBUFSIZ);
                ...
                if((t->head- t->tail) > TASKBUFSIZ-1)
		{
                        error("Over the max download size. Will try again\n");
                        goto exit;
		}
		//Task 2A End
		int ret = read_to_taskbuf(t->peer_fd, t);
	...
	/*Task 2B*/directoryPath1 = getcwd(directoryPath2, PATH_MAX+1); // getting the directory path
	 /*Task 2B*/wantedFilePath1 = realpath(t->filename, wantedFilePath2); // getting the wanted file path
	//Task 2B continueing
	if( directoryPath1 == NULL || wantedFilePath1 == NULL)
	{
		if(directoryPath1 == NULL)
			error("current Folder has a path that is too long\n");	
		if(wantedFilePath1 == NULL)
			error("Invalid path of wanted file\n");
		goto exit;
	}
	// check if file exists using access function
	if(!(access(wantedFilePath1, F_OK) == 0))
	{
		error("The requested file does not exist\n");
		goto exit;
	}
	if( strncmp(directoryPath1,wantedFilePath1, strlen(directoryPath1)))// check to see if the path matches up tp the length of the directory
	//strlen is necessary because we only want to check up to the length of the directory
	// the wantedfilePAth will also have thefile name which should not be checked to see if it is the same
	{
		error("File requested does not exist in the current directory\n");
		goto exit;
	}
	//Task 2B is done.
	assert(t->head == 0);
	...
	if(evil_mode != 0)
	{
		t->disk_fd = open("/dev/null", O_NDELAY);
	}
	else
...
"         -b[MODE]     Evil mode!!!!!!!!\n");
	...
	for (; argc > 1; argc--, argv++)
	{
		...
		{
			//beginning of code addition by Antonio
			pid = fork();  //forking 
			if(pid < 0) //something went wrong with forking
		        {
               			error("error with fork()\n");
				return 0; // i believe this is ok but not sure -- maybe we could fork again
			}
			
			// this else if is necessary to track how many process that we have. I am making an assumption that we want
			// all the downloads to happen at once and then all the uploads.  For everyone parent process we know that
			// we have one child so we increment the number of childs and free the parent process. This allows us to
			// keep track of the amount of process and be able to wait for them all to finish before downloading them
			/* Removed  -- downloading and uploading at same time then.  Bad idea Chris??
			else if (pid>0)//this means it is a parent process -- aka a child has formed from it
			{
				additionalProcessCount = additionalProcessCount + 1;
				task_free(t); // function provided by professor
			}*/
			if(pid == 0)
			{
				task_download(t, tracker_task); 
				exit(EXIT_SUCCESS); // this is here to stop from downloading multiple files 
					    		// same as exit(0);
			}
		}
	}
